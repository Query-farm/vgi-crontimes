#!/usr/bin/env bash
# Build the crontimes VGI worker and run the SQLLogic tests against it using the
...
echo "==> Building crontimes-worker (release)"
cargo build --release --bin crontimes-worker
...
WORKER="$REPO_ROOT/target/release/crontimes-worker"
...
VGI_WORKER_CATALOG_NAME="crontimes" \
